...
#Se intenta revisar si es que hay algunos caminos innecesarios
for j,routes in enumerate(vrp):
  indAux=0
  g = groupNodes[j]
  n =len(g)-1
  revisionExtra=[False]*n
  for ind,route in enumerate(routes):    
    if revisionExtra.count(False)==0:
      indAux=ind
      break
    for i in range(n):
      if g[i] in route:
        revisionExtra[i]=True
  auxN=len(routes)-1
  if indAux != auxN:
    while auxN>indAux:
